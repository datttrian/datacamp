...
filter, arrange, and mutate. You’ll use these functions to modify the
counties dataset to view particular observations and answer questions
about the data.
...
- Use `mutate()` to add a column called `public_workers` to the dataset,
  with the number of people employed in public (government) work.
...
Now that you know how to transform your data, you’ll want to know more
about how to aggregate your data to make it more interpretable. You’ll
learn a number of functions you can use to take many observations in
your data and summarize them, including count, group_by, summarize,
ungroup, and top_n.
...
- Use `mutate()` to calculate and add a column called `population_walk`,
  containing the total number of people who walk to work in a county.
...
- Group the data by state, and summarize to create the columns
  `total_area` (with total area in square miles) and `total_population`
  (with total population).
- Add a `density` column with the people per square mile, then arrange
  in descending order.
...
- Summarize to find the total population, as a column called
  `total_pop`, in each combination of region and state, grouped in that
  order.
- Notice the tibble is still grouped by region; use another
  `summarize()` step to calculate two new columns: the average state
  population in each region (`average_pop`) and the median state
  population in each region (`median_pop`).
...
- Find the county in each region with the highest percentage of citizens
  who walk to work.
...
- Calculate the average income (as `average_income`) of counties within
  each region and state (notice the `group_by()` has already been done
  for you).
...
Learn advanced methods to select and transform columns. Also learn about
select helpers, which are functions that specify criteria for columns
you want to choose, as well as the rename and transmute verbs.
...
- Select the columns for `state`, `county`, `population`, and (using a
  ...
  consecutive variables in the table related to the industry of people’s
  work: `professional`, `service`, `office`, `construction`, and
  `production`.
...
- Select the columns `state`, `county`, `population`, and all those that
  end with `work`.
...
- Notice the `n` column in the output; use `rename()` to rename that to
  `num_counties`.
...
Which of the following verbs would you use to calculate new columns
while dropping other columns?
...
- Keep only the `state`, `county`, and `population` columns, and add a
  new column, `density`, that contains the `population` per `land_area`.
...
We’ve learned a number of new verbs in this chapter that you can use to
modify and change the variables you have.
...
- Choose the right verb for keeping only the columns `state`, `county`,
...
Work with a new dataset that represents the names of babies born in the
United States each year. Learn how to use grouped mutates and window
functions to ask and answer more complex questions about your data. And
use a combination of dplyr and ggplot2 to make interesting graphs to
...
- Use `group_by()` and `slice_max()` to find the most common name for US
  babies in each year.
...
- Visualize those three names as a line plot over time, with each name
...
- First, calculate the total number of people born in that year in this
  dataset as `year_total`.
- Next, use `year_total` to calculate the `fraction` of people born in
  each year that have each name.
- Now use your newly calculated `fraction` column, in combination with
  `slice_max()`, to identify the year each name was most common.
...
- Use a grouped mutate to add two columns:
  - `name_total`: the sum of the number of babies born with that name in
    the entire dataset.
  - `name_max`: the maximum number of babies born with that name in any
    year.
- Add another step to ungroup the table.
- Add a column called `fraction_max` containing the `number` in the year
  divided by `name_max`.
...
In this course, you’ve learned how to transform data with the dplyr
package. You reviewed the select, filter, mutate, and arrange verbs for
transforming data. You mastered the count verb, and learned the powerful
group_by and summarize pattern for aggregating data.

### Verbs table

You learned four verbs you can use for adding, removing, and changing
variables in a table: select, rename, transmute, and mutate.

### babynames data

You also applied many of these tools to a new babynames dataset, and
gained experience with grouped mutates and window functions.

### Other DataCamp courses

You’ll find all of these skills valuable in your own projects, as well
as in countless other DataCamp courses, including more intermediate and
advanced dplyr courses, and courses that combine dplyr with other
Tidyverse packages.

### Congratulations!

Congratulations again, and best of luck using your new-found dplyr
skills in your own analyses!
